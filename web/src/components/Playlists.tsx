  ...
  return (
    <section className="card">
      ...
      <div className="add-song-btns">
        ...
        />
        <button className="wide-btn accent" onClick={create}>+ {t('ui_new_playlist')}</button>
      </div>
      <div className="playlist-list">
        {playlists.map((pl) => (
          <div className="playlist-item" key={pl.id}>
            <div className="playlist-head" onClick={() => setExpanded(expanded === pl.id ? null : pl.id)}>
              <span className="playlist-name">{pl.name}</span>
              <span className="playlist-count">{pl.tracks.length} {t('ui_songs')}</span>
            </div>
            <div className="playlist-actions">
              <button className="mini-btn" onClick={() => control('playlistLoad', { id: pl.id })} title={t('ui_load_playlist')}>⏏</button>
              {current && (
                <button className="mini-btn" onClick={() => onAddCurrent(pl.id, current)} title={t('ui_add_current')}>+♪</button>
              )}
              <button className="mini-btn danger" onClick={() => control('playlistDelete', { id: pl.id })} title={t('ui_delete')}>🗑</button>
            </div>
              ...
              <div className="playlist-tracks">
                {pl.tracks.map((track, i) => (
                  <div className="queue-item small" key={`${track.url}-${i}`}>
                    ...
                    <button className="mini-btn" onClick={() => control('playTrack', { track })} title={t('ui_play_now')}>▶</button>
                    <button
